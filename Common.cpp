	...
	, OptionsFile(json::parse(std::fstream{ options_file_path }))
{
	...
	OPTION(ModificationTimeDatabasePath, path{ OptionsFilePath }.concat("mtd").string(), "The path to the modification-time database; it's usually placed next to the options file, but in case you have two different options files that want to use the same database, this option is available");
}
